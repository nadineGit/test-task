 ...
 * Correct `button` scss inheritance in Firefox, IE 8/9/10/11, and Opera.
 * Correct `select` scss inheritance in Firefox.
 */
button, select {
  text-transform: none; }
 ...
 *    and `video` controls.
 * 2. Correct inability to scss clickable `input` types in iOS.
 * 3. Improve usability and consistency of cursor scss between image-type
 *    `input` and others.
 */
...
/**
 * Fix the cursor scss for Chrome's increment/decrement buttons. For certain
 * `font-size` values of the `input`, it causes the cursor scss of the
